...
def parse_time(time_str: str) -> datetime:
    ...
    ----------
    time_str : str
        Time string of length 4, 6, 7, or 8.

    Returns
    -------
    datetime
        datetime object with date components set to 1900-01-01 (default).
        Use parse_datetime() to combine with a date.

    Raises
    ------
    ValueError
        If the string length is not 4, 6, 7, or 8, or the time is not valid.
...
def parse_datetime(date_str: str, time_str: str) -> datetime:
    ...
    Examples
    --------
    >>> parse_datetime('20060101', '1200')
    datetime.datetime(2006, 1, 1, 12, 0)

    >>> parse_datetime('2006-01-01', '120000')
    datetime.datetime(2006, 1, 1, 12, 0, 0)

    >>> parse_datetime('01/01/2006', '12000055').isoformat()
